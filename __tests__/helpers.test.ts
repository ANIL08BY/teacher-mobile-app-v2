...
describe('2. Yardımcı Fonksiyon (helpers.ts) Mantık Testleri', () => {
  ...
  it('2. İsim veya soyisim eksikse hata yerine "??" dönmelidir', () => {
    expect(generateInitials('', '')).toBe('??');
  });

  it('3. Telefon numarası 10 haneden büyükse sistem geçerli saymalıdır', () => {
    expect(validatePhoneNumber('05554443322')).toBe(true);
  });

  it('4. Telefon numarası eksik veya kısaysa güvenlik için geçersiz sayılmalıdır', () => {
